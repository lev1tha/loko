...
1. **Мост** (сейчас): PHP пишет в свой MySQL, Loko забирает изменения cron-ом
   (`import_kargoosh --incremental` каждые 5 мин, `--rescan` ночью). Loko-панель
   показывает данные Kargo только для чтения.
